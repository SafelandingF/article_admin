...
const { port, uploadDst } = config.server
...
app.use((req, res, next) => {
    ...
    if (whiteList.indexOf(location) > -1) {
        ...
        if (typeof authorization === 'undefined') {
            ...
            token.verifyToken(authorization).then(data => {
                req.data = data
                return next()
            }).catch(error => {
...
const findTag = async (tag) => {
    // 查找标签是否已存在，如果没有就添加一条
    const sql_tag_find = `select * from tag where tag_name = ?;`
    return db(sql_tag_find, tag).then(res => {
        ...
        if (result.length > 0) {
            ...
            // 增加一条新标签
            const sql_tag_insert = `insert into tag(tag_name) values(?);`
...
app.post('/api/addArticle', async (req, resp) => {
    ...
    for (const tag of params.tagName) {
        const id = await findTag(tag)
        tagID.push(id)
    }

    const res = await db(sql_insert, sqlParam)
    ...
    db(sql_find, findParams).then(
        async res => {
            if (res.length > 0) {
                ...
                const registerRes = await db(sql_add, addParams)
                const sql_add_default_collection = `insert into collection(user_id, collection_name, collection_description) values(${registerRes.insertId}, "默认收藏", "默认收藏夹")`
                const collectionRes = await db(sql_add_default_collection)
...
app.post('/api/userInfoChange', (req, resp) => {
    ...
    const resValue = { result: 0 }
    db(sql_find, params.id).then(res => {
        const checkID = JSON.parse(JSON.stringify(res))[0].user_id
...
app.post('/api/uploadAvatar', upload.single('avatar'), (req, resp) => {
    const file = req.file
    const savingName = file.filename + new Date().getTime()
    const rawSrc = 'upload/UserAvatar/' + file.filename
    const realSrc = 'upload/UserAvatar/' + savingName
...
app.post('/api/imgUpload', uploadImg.single('image'), (req, resp) => {
    ...
    const savingName = file.originalname.split('.')[0] + new Date().getTime() + '.' + mimeType
    const rawSrc = 'public/' + file.filename
    const realSrc = 'public/' + savingName
    fs.renameSync(rawSrc, realSrc)
    resp.send(uploadDst + 'public/' + savingName)
})
...
app.post('/api/favoriteStatus', (req, resp) => {
    ...
    db(sql_find).then(async res => {
        ...
        if (res.length > 0) {
            ...
            // 先找到该用户的默认收藏夹
            const sql_find_default_collection = `select collection_id from collection where user_id = ${params.userID} and collection_name="默认收藏";`
            const collectionRes = await db(sql_find_default_collection)
            const cid = JSON.parse(JSON.stringify(collectionRes))[0].collection_id
            // 插入到默认收藏夹中
...
app.get('/api/CommentsReceived', (req, resp) => {
    ...
    where co.article_commented_id = ar.id;`
    const p1 = db(sql_comment)
    const p2 = db(sql_reply)
...
app.post('/api/articleFavorite', (req, resp) => {
    ...
    const sql_clearStatus = `update article_favorite set favorite_status = 0
...
app.post('/api/Login', (req, resp) => {
    ...
    db(sql, sqlParam).then(
        res => {
            if (res.length) {
                const value = JSON.parse(JSON.stringify(res))
                const pwd = value[0].user_pwd
